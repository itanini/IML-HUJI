...
class LogisticRegression(BaseEstimator):
    ...
    def _fit(self, X: np.ndarray, y: np.ndarray) -> NoReturn:
        ...
        d = X.shape[1]
        init = np.random.normal(0, 1, d) / np.sqrt(d)

        if self.penalty_ == 'l1':
            regul = RegularizedModule(fidelity_module=LogisticModule(init), regularization_module=L1(init),
                                      weights=init, lam=self.lam_, include_intercept=self.include_intercept_)
            self.coefs_ = self.solver_.fit(regul, X, y)

        elif self.penalty_ == 'l2':
            regul = RegularizedModule(fidelity_module=LogisticModule(init), regularization_module=L2(init),
                                      weights=init, lam=self.lam_, include_intercept=self.include_intercept_)
            self.coefs_ = self.solver_.fit(regul, X, y)

        elif self.penalty_ == 'none':
            self.coefs_ = self.solver_.fit(f=LogisticModule(init), X=X, y=y)
    ...
    def predict_proba(self, X: np.ndarray) -> np.ndarray:
        ...
        if self.include_intercept_:
            X = np.insert(X, 0, 1, axis=1)
        seg = lambda x: np.exp(x)/(np.exp(x) + 1)
        return seg(X @ self.coefs_)
