...
class GradientDescent:
    ...
    def fit(self, f: BaseModule, X: np.ndarray, y: np.ndarray):
        ...
        iter = 0
        while True:
            w_t_m_1 = f.weights  # stands for w(t-1)
            f.weights = f.weights - self.learning_rate_.lr_step(t=iter) * f.compute_jacobian(X=X, y=y)
            self.callback_[0](model=self, weights=f.weights, val=f.compute_output(X=X, y=y),
                              grad=f.compute_jacobian(X=X, y=y), t=iter,
                              eta=self.learning_rate_.lr_step(t=iter), delta=np.linalg.norm(f.weights - w_t_m_1))
            if self.out_type_ == 'best' and output > f.compute_output(X=X, y=y):
                output = f.weights
                ...
                output = f.weights
            iter += 1
            if np.linalg.norm(f.weights - w_t_m_1) < self.tol_ or iter == self.max_iter_:
