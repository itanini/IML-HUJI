...
class L1(BaseModule):
    ...
    def compute_output(self, **kwargs) -> np.ndarray:
        ...
        output: ndarray of shape (1,)
            Value of function at point self.weights
        """
        return np.sum(np.abs(self.weights.copy()))
    ...
    def compute_jacobian(self, **kwargs) -> np.ndarray:
        ...
        """
        return np.sign(self.weights.copy())
...
class LogisticModule(BaseModule):
    ...
    def compute_jacobian(self, X: np.ndarray, y: np.ndarray, **kwargs) -> np.ndarray:
        ...
        return (-1 / X.shape[0]) * ((y - (e_Xw / (1 + e_Xw))) @ X)
...
class RegularizedModule(BaseModule):
    ...
    def compute_output(self, **kwargs) -> np.ndarray:
        ...
        output: ndarray of shape (1,)
            Value of function at point self.weights
        """
        fidelity = self.fidelity_module_
        regularization = self.regularization_module_
        return fidelity.compute_output(X=kwargs['X'], y=kwargs['y']) + self.lam_ * regularization.compute_output()
    ...
    def compute_jacobian(self, **kwargs) -> np.ndarray:
        ...
        return fidelity.compute_jacobian(X=kwargs['X'], y=kwargs['y']) + self.lam_ * regularization.compute_jacobian()
